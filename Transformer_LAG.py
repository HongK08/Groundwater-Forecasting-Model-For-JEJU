...
NHEAD = 4
NUM_LAYERS = 2
RES_PATH = "/media/user/AI_2T/UML_Paper/Daytime/Transformer_LAG"
...
def main():
    ...
    FEATURE_COLS_EXT = FEATURE_COLS + [f"{col}_lag{lag}" for col in TARGET_COLS for lag in [1, 2, 3]]
    ...
    df_result.to_csv(f"{RES_PATH}/transformer_prediction_summary.csv", index=False)
    pd.DataFrame(all_preds, columns=["Well", "Horizon", "Date", "True", "Pred"]).to_csv(f"{RES_PATH}/transformer_predictions.csv", index=False)

    os.makedirs(f"{RES_PATH}/plots", exist_ok=True)
    with PdfPages(f"{RES_PATH}/plots/transformer_report.pdf") as pdf:
        ...
        for m in ['MAE', 'MSE', 'RMSE', 'R2']:
            ...
            plt.title(f"{m} Heatmap (Transformer + LAG)")
            plt.xlabel("Horizon")
            plt.ylabel("Well")
            plt.tight_layout()
            pdf.savefig(); plt.close()

        pred_df = pd.read_csv(f"{RES_PATH}/transformer_predictions.csv")
