...
res_path = '/media/user/AI_2T/UML_Paper/Daytime/BiLSTM_LAG'
...
def run_bilstm_pipeline():
    ...
    pred_df.to_csv(f'{res_path}/bilstm_predictions.csv', index=False)

    summary_df = pd.DataFrame(metric_table, columns=["Well", "Horizon", "MAE", "MSE", "RMSE", "R2"])
    summary_df.to_csv(f'{res_path}/bilstm_prediction_summary.csv', index=False)

    # PDF 리포트 생성
    pdf_path = f'{res_path}/plots/bilstm_report.pdf'
    ...
    print(f"\n[Done] BiLSTM Report saved to: {pdf_path}")
