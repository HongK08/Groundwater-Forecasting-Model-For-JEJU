...
# ====== 설정 ======
data_path = '/media/user/AI_2T/UML_Paper/Main_Data_Jeju/JeJu_merged.csv'  # 새 데이터셋 경로
res_path = '/media/user/AI_2T/UML_Paper/Daytime/'
...
def run_lstm_pipeline():
    ...
    for target_col in TARGET_COLS:
        for horizon in HORIZONS:
            ...
            model = LSTMModel(X_scaled.shape[1], HIDDEN_DIM, 1).to(device)
    ...
    pred_df.to_csv(f'{res_path}/lstm_predictions_layer1.csv', index=False)

    summary_df = pd.DataFrame(metric_table, columns=["Well", "Horizon", "MAE", "MSE", "RMSE", "R2"])
    summary_df.to_csv(f'{res_path}/lstm_prediction_summary_layer1.csv', index=False)

    # PDF 리포트 생성
    pdf_path = f'{res_path}/plots/lstm_layer1_report.pdf'
    ...
    print(f"\n[Done] LSTM Layer=1 Report saved to: {pdf_path}")
